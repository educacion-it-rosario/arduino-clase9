...
class Context
{
   ...
   public static $gianas = array();
    ...
    // -------------------------------------------------------------------------
    const servicesRootURI = "http://localhost:8000/giana/srv/"; // |
    // -------------------------------------------------------------------------
    ...
    public static function initialize()
    {
        ...
        foreach (glob("/dev/ttyUSB*") as $filename) {
            array_push(Context::$gianas, new Giana($filename));
        }
    }
    ...
    public static function getServicesRootURI()
    {
        ...
        return Context::servicesRootURI .
            (substr (
                Context::servicesRootURI,
                strlen(Context::servicesRootURI) - 1,
                1
            ) == "/" ? "" : "/");
    ...
    public static function getFirstNode($dom, $name) {
        return $dom->getElementsByTagName($name)->item(0);
    }

    public static function firstNodeValue($dom, $name) {
        return Context::getFirstNode($dom, $name)->nodeValue;
    }
...
class Giana
{
    ...
    private function get($request)
    {
        ...
        $serial->deviceOpen();
        Log::this("Sending: ". $message, false);
        $serial->sendMessage($message."\r\n");
        sleep(1);

        $message = $serial->readPort();
        Log::this("Received: " . $message, false);
        ...
        $response = new Response (
                ...
                ($request->pinType == PinType::Digital ? 1 : 4))
            );
...
class Schedule
{
    ...
    public static function get()
    {
        ...
        if ($domDocument->getElementsByTagName("device")->length > 0)
        {
            ...
            $schedule->id = Context::getFirstNode($domDocument, "schedule")->
                getAttribute("id");
            $schedule->port = Context::firstNodeValue($domDocument, "port");
            $schedule->pinType = Context::firstNodeValue($domDocument,
                                                         "pinType");
            $schedule->pinNumber = Context::firstNodeValue($domDocument,
                                                           "pinNumber");
            $schedule->trueValue = Context::firstNodeValue($domDocument,
                                                           "trueValue");
            $schedule->falseValue = Context::firstNodeValue($domDocument,
                                                            "falseValue");
    ...
    public function done($error)
    {
        ...
        try {
            ...
            REST::this(
                Context::getServicesRootURI() . "schedule/?scheduleId=" .
                $this->id .
                "&error=" . ($error ? "Y" : "N"),
                "POST");
...
class Request
{
    ...
    public static function get()
    {
        ...
        if ($domDocument->getElementsByTagName("action")->length > 0)
        {
            ...
            $request = new Request(
                Context::firstNodeValue($domDocument, "requestId"),
                Context::firstNodeValue($domDocument, "port"),
                Context::firstNodeValue($domDocument, "action"),
                Context::firstNodeValue($domDocument, "pinType"),
                Context::firstNodeValue($domDocument, "pinNumber"),
                Context::firstNodeValue($domDocument, "value")
            );
